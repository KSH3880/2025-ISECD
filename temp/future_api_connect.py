...
def process_edges_parallel_future(edges_df, api_keys, departure_time, max_workers=10):
    results = []
    failures = []
    ...
    def task(row_index_row):
        ...
        result = get_future_route_info_with_rotation(origin, destination, api_keys, idx, row['u'], row['v'], departure_time)
...
if __name__ == "__main__":
    ...
    # 미래 출발 시간 설정 (예: 2025년 7월 1일 오전 8시)
    departure_time = "202505280230"

    # 🔁 1차 병렬 실행
    success_df, fail_df = process_edges_parallel_future(filtered_edges, api_keys, departure_time, max_workers=10)
    ...
    # CSV 저장
    assigned_edges.to_csv("../data/assigned_edges_future.csv", index=False, encoding='utf-8-sig')
